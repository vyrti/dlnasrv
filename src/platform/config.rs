...
impl PlatformConfig {
    ...
    fn for_windows() -> Self {
        ...
        let local_app_data = dirs::data_local_dir().unwrap_or_else(|| home_dir.join("AppData\\Local"));
    ...
    pub fn ensure_directories_exist(&self) -> PlatformResult<()> {
        std::fs::create_dir_all(&self.config_dir)
            .map_err(|e| PlatformError::FileSystemAccess(format!("Failed to create config directory: {}", e)))?;
        
        std::fs::create_dir_all(&self.log_dir)
            .map_err(|e| PlatformError::FileSystemAccess(format!("Failed to create log directory: {}", e)))?;
        
        std::fs::create_dir_all(&self.cache_dir)
            .map_err(|e| PlatformError::FileSystemAccess(format!("Failed to create cache directory: {}", e)))?;
        
        std::fs::create_dir_all(&self.database_dir)
            .map_err(|e| PlatformError::FileSystemAccess(format!("Failed to create database directory: {}", e)))?;
    ...
    pub fn is_case_sensitive(&self) -> bool {
        self.metadata.get("case_sensitive")
            .map(|v| v == "true")
            .unwrap_or(false)
    }
    ...
    pub fn get_path_separator(&self) -> &str {
        self.metadata.get("path_separator")
            .map(|s| s.as_str())
            .unwrap_or("/")
    }
    ...
    pub fn supports_network_paths(&self) -> bool {
        self.metadata.get("supports_unc_paths")
    ...
    pub fn get_default_media_extensions(&self) -> Vec<String> {
        ...
        let mut extensions = vec![
            ...
            "webm".to_string(), "m4v".to_string(), "mpg".to_string(),
            "mpeg".to_string(), "3gp".to_string(), "ogv".to_string(),
            
            // Audio formats
            "mp3".to_string(), "flac".to_string(), "wav".to_string(),
            "aac".to_string(), "ogg".to_string(), "wma".to_string(),
            "m4a".to_string(), "opus".to_string(), "ape".to_string(),
            
            // Image formats
            "jpg".to_string(), "jpeg".to_string(), "png".to_string(),
            "gif".to_string(), "bmp".to_string(), "webp".to_string(),
            "tiff".to_string(), "svg".to_string(),
        ];
    ...
    pub fn validate_path(&self, path: &PathBuf) -> PlatformResult<()> {
        ...
        match self.os_type {
            OsType::Windows => {
                ...
                for &ch in &invalid_chars {
                    if path_str.contains(ch) {
                        return Err(PlatformError::FileSystemAccess(
                            format!("Path contains invalid Windows character '{}': {}", ch, path.display())
                        ));
                    }
                }
                ...
                if let Some(file_name) = path.file_name() {
                    ...
                    for reserved in &reserved_names {
                        if name == *reserved || name.starts_with(&format!("{}.", reserved)) {
                            return Err(PlatformError::FileSystemAccess(
                                format!("Path uses reserved Windows name: {}", path.display())
                            ));
                        }
                    }
                }
            ...
            OsType::MacOS => {
                ...
                // Check for colon in filename (not allowed in macOS)
                if let Some(file_name) = path.file_name() {
                    if file_name.to_string_lossy().contains(':') {
                        return Err(PlatformError::FileSystemAccess(
                            format!("Path contains colon in filename (not allowed on macOS): {}", path.display())
                        ));
                    }
                }
                
                // Check path length (macOS has a 255 character limit for filenames)
                if let Some(file_name) = path.file_name() {
                    if file_name.len() > 255 {
                        return Err(PlatformError::FileSystemAccess(
                            format!("Filename too long (>255 characters): {}", path.display())
                        ));
                    }
                }
            }
            ...
            OsType::Linux => {
                // Check for null bytes (not allowed in Linux paths)
                let path_str = path.to_string_lossy();
                ...
                // Check path length (Linux has a 255 character limit for filenames)
                if let Some(file_name) = path.file_name() {
                    if file_name.len() > 255 {
                        return Err(PlatformError::FileSystemAccess(
                            format!("Filename too long (>255 characters): {}", path.display())
...
mod tests {
    ...
    fn test_path_validation() {
        ...
        assert!(config.validate_path(&current_dir).is_ok());
        
        // Test with non-existent path
        let non_existent = PathBuf::from("/this/path/should/not/exist/hopefully");
        assert!(config.validate_path(&non_existent).is_err());
    }
}